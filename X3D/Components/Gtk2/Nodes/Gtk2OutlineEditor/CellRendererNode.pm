package Gtk2OutlineEditor::CellRendererNode;
use strict;
use warnings;
...
use Glib::Object::Subclass Gtk2::CellRenderer::,
  properties =>
  [ Glib::ParamSpec->scalar( "node", "X3DNode", "Description", [qw(readable writable)] ) ],
  ;

use constant x_padding => 2;
use constant y_padding => 1;

use constant icon_width  => 10;
...
sub format_text {
	my $cell = shift;
	my $node = $cell->{node}->getValue;
	my $name = $node->getName;
	return sprintf "<b>%s</b> %s [%d]", $node->getType, $name, $cell->{node}->getReferenceCount
	  if $name;
	return sprintf "<b>%s</b> [%d]", $node->getType, $cell->{node}->getReferenceCount;
}
...
sub RENDER {
	...
	$window->draw_pixbuf(
		$widget->get_style->fg_gc( $widget->state ), $cell->{icon}, 0, 0,
